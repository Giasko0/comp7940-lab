...
def save_message_async(self, user_id, message, sender_type="user"):
    from Mongo_db import save_chat_message

    try:
        result = save_chat_message(user_id, message, sender_type)
...
@app.task(bind=True, max_retries=3)
def process_search_async(self, query, user_id):
    from Search import search_similar_users

    try:
        results = search_similar_users(query)
...
@app.task(bind=True, max_retries=3)
def generate_chat_response_async(self, message, user_id):
    ...
    try:
        ...
        logger.error("Failed to generate response for user %s: %s", user_id, exc)
        raise self.retry(exc=exc, countdown=2 ** self.request.retries)
